...
import timm
from torch_utils.models.layers import GeM
...
class DolgLocalBranch(nn.Module):
    def __init__(self, image_size, in_channel, out_channel, hidden_channel=2048):
        super().__init__()
...
class DolgNet(nn.Module):
    def __init__(self, backbone, pretrained, image_size, input_dim, hidden_dim, output_dim):
        super().__init__()
        ...
        self.local_branch = DolgLocalBranch(image_size, 512, hidden_dim)  # 512 is decided by model
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.gem_pool = GeM()
        self.fc_1 = nn.Linear(1024, hidden_dim)  # 1024 is decided by model
