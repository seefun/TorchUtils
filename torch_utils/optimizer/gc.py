# modified from https://github.com/Yonghongwei/Gradient-Centralization

# GCC: GC for conv only
# GC: GC for both conv and fc
# weight decay: 5e-5 or 1e-4; momentum: 0.9
...
class SGD_GCC(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0.9, dampening=0,
                 weight_decay=5e-5, nesterov=False):
...
class AdamW_GCC2(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=5e-5, amsgrad=False):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps:
